...
class Generator(nn.Module):
    def __init__(
        ...
        if self.label_size > 0:
            print("Detected conditional model, initializing LabelConcat layer..")
        ...
        for i in range(n_mlp):
            if self.label_size > 0 and i==0:
                ...
                input_dim = style_dim
            layers.append(EqualLinear(input_dim, style_dim, lr_mul=lr_mlp, activation='fused_lrelu'))
    ...
    def get_latent(self, styles, conditioning = None, normalize = True):
        if self.label_size > 0:
            if conditioning is None:
                conditioning = torch.zeros(styles.shape[0], self.label_size, device = self.input.input.device)
            label_embedding = self.label_concat(conditioning)
            styles = torch.cat((styles, label_embedding), dim=1)

        if normalize:
            styles = styles / torch.sqrt(torch.mean((styles)**2, dim=1, keepdim=True) + 1e-8)
    ...
    def forward(
        ...
        if truncation != 1:
            if truncation_latent is None:
                raise("Applying truncation, but truncation_latent not given!!")
            styles = truncation*styles + (1-truncation) * truncation_latent

        if styles.shape[1] != self.n_latent:
...
class Discriminator(nn.Module):
    def __init__(self, size, label_size = 0, channel_multiplier=2, blur_kernel=[1, 3, 3, 1]):
        super().__init__()
        ...
        self.final_linear = nn.Sequential(
            ...
            EqualLinear(channels[4], max(self.label_size, 1)),
        )

    def forward(self, input, labels_in = None):
        ...
        if self.label_size > 0:
            print("WARNING:")
            print("Conditional implementation of D needs to be validated! Could contain errors!!!")
            out = torch.sum(out * labels_in, dim=1, keepdims=True)
