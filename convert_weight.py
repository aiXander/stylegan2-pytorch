...
python3 convert_weight.py --repo /home/rednax/Desktop/music_vr/StyleGAN_training/stylegan2 /home/rednax/Desktop/music_vr/Great_Models/stylegan2-ffhq-config-f.pkl

Collection:
cd /home/rednax/Desktop/GitHub_Projects/stylegan2-pytorch/
python3 convert_weight.py --repo /home/rednax/Desktop/music_vr/StyleGAN_training/stylegan2 /home/rednax/Desktop/music_vr/StyleGAN_training/stylegan2/results/00140-stylegan2-collection-1gpu-cond-config-f/network-snapshot-002518.pkl

to try:
https://github.com/huangzh13/StyleGAN2.pytorch
'''
...
def convert_dense(vars, source_name, target_name, bias = True):
    weight = vars[source_name + '/weight'].value().eval()
    ...
    if bias:
        ...
        dic = {'weight': weight.transpose((1, 0))}
...
def fill_statedict(state_dict, vars, size, label_size):
    log_size = int(math.log(size, 2))

    if label_size > 0:
        update(state_dict, convert_dense(vars, f'G_mapping/LabelConcat', 'label_concat', bias=False))
    ...
    update(
        state_dict,
        {'input.input': torch.from_numpy(vars['G_synthesis/4x4/Const/const'].value().eval())},
    )
...
def get_tf_model_info(Gs_network, verbose = 1):
...
if __name__ == '__main__':
    ...
    print("Loading TF model from pkl...")
    with open(args.path, 'rb') as f:
        generator, discriminator, g_ema = pickle.load(f)

    _N, (w,h,c), w_dim, label_size, dlatent_avg = get_tf_model_info(g_ema)
    size = g_ema.output_shape[2]

    print("Creating PyTorch generator...")
    g = Generator(size, 512, 8, label_size = label_size, channel_multiplier=args.channel_multiplier)
    state_dict = g.state_dict()

    print("Overwriting generator weights with TF weights..")
    state_dict = fill_statedict(state_dict, g_ema.vars, size, label_size)
    g.load_state_dict(state_dict)

    dlatent_avg = torch.from_numpy(g_ema.vars['dlatent_avg'].value().eval())
    ckpt = {'g_ema': state_dict, 'dlatent_avg': dlatent_avg}

    if args.gen:
        print("Creating g..")
        g_train = Generator(size, 512, 8, label_size = label_size, channel_multiplier=args.channel_multiplier)
        g_train_state = g_train.state_dict()
        g_train_state = fill_statedict(g_train_state, generator.vars, size, label_size)
        ckpt['g'] = g_train_state

    if args.disc:
        print("Creating d..")
        disc = Discriminator(size, label_size = label_size, channel_multiplier=args.channel_multiplier)
    ...
    z = np.random.randn(n_sample, 512).astype('float32')
    if label_size > 0:
        conditioning    = np.random.randn(n_sample, label_size).astype('float32')
        conditioning_pt = torch.from_numpy(conditioning).to(device)
        print("Generating conditional images!")
    else:
        conditioning    = None
        conditioning_pt = None

    print("Rendering PyTorch images..")
    with torch.no_grad():
        img_pt, _ = g(
            torch.from_numpy(z).to(device),
            conditioning = conditioning_pt,
            truncation=sample_truncation,
            truncation_latent=dlatent_avg.to(device),
        )

    print("Rendering TF images..")
    Gs_kwargs = dnnlib.EasyDict()
    Gs_kwargs.randomize_noise = False
    img_tf = g_ema.run(z, conditioning, truncation_psi = sample_truncation, **Gs_kwargs)
    ...
    print("Saving comparison images to %s" %name)
